...
最新仕様の代表的なサンプル 18 本を、コード・出力・メリット・デメリットとともに解説し、
そのうち 16 本が\textbf{三つの独立な処理系で出力一致}することを確認した
（\texttt{s11} と \texttt{s16} は出力にノード名を含むため自動比較から外し、内容を目視で照合した）。
弾かれるべきプログラムは 29 本になった。
\end{abstract}
...
後者は\textbf{29 本}の「弾かれるべきプログラム」からなる。
...
サンプル 18 本のうち\textbf{16 本が三実装で出力一致}する
（\texttt{s11} と \texttt{s16} は出力にノード名 \texttt{rt0/servo1} を含み、
比較スクリプトが区切りに使う \texttt{/} で割れてしまうため自動比較から外した。
内容は目視で一致を確認している）。
ガイドの 8 本も一致する。
弾かれるべき 29 本については、87 枠のうち\textbf{10 枠}が期待と食い違い、
その内訳は JS 版 7・Py 版 3 である（\textbf{OCaml 版は 29 本すべてで期待どおりに振る舞う}）。
...
現状は、\textbf{機械検証の代わりに「弾かれるべきプログラム」29 本}が
実装の規律を押さえている。
...
\begin{enumerate}
\item \textbf{資源への全体順序}。
      \texttt{acquire} / \texttt{release} は対を見るが、
      資源のあいだに順序が無いので、取得順序の食い違いによる
      デッドロックが残る。
      \S\ref{sec:dl3} の義務レベルを資源にも課すのが素直な道である。
