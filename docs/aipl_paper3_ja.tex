...
併せて、効果の伝播に使う辺がそのまま待ちグラフであることに気づき、
\textbf{循環待ちの静的検査}をほぼ費用ゼロで加えた。
ただし\textbf{デッドロック自由は型で保証されない}。
型検査を通るのに詰まる 10 行のプログラムを本文に示し、
保証には義務レベルが要ることを述べる。

採否の理由はすべて動く最小プログラムで示す。
最新仕様の代表的なサンプル 14 本を、コード・出力・メリット・デメリットとともに解説し、
そのうち 13 本が\textbf{三つの独立な処理系で出力一致}することを確認した。
\end{abstract}
...
\paragraph{ただしデッドロック自由は保証されない。}
これは強調しておく必要がある。次の 10 行は\textbf{型検査を通り}、実行すると詰まる。
...
型で保証するには\textbf{義務レベル}が要る ----
各メソッドにレベルを付け、\texttt{now}/\texttt{await} は
厳密に大きいレベルにしか向かえない、とすれば
待ちのグラフが構成的に非巡回になる。
資源の側も、全体の順序を課さなければ取得順序のデッドロックが残る。
これが次の課題である。
...
後者は\textbf{21 本}の「弾かれるべきプログラム」からなる。
...
サンプル 14 本のうち\textbf{13 本が三実装で出力一致}する
（残る 1 本 \texttt{s11} は配備の出力にノード名を含むため比較の対象から外した）。
ガイドの 8 本も一致する。
弾かれるべき 21 本については、63 枠のうち\textbf{10 枠}が期待と食い違い、
その内訳は JS 版 7・Py 版 3 である（\textbf{OCaml 版は 21 本すべてを弾く}）。
...
現状は、\textbf{機械検証の代わりに「弾かれるべきプログラム」21 本}が
実装の規律を押さえている。
...
\begin{enumerate}
\item \textbf{義務レベルによるデッドロック自由}。
      \texttt{now}/\texttt{await} が厳密に大きいレベルにしか向かえないようにする。
      資源にも全体の順序を課す。
