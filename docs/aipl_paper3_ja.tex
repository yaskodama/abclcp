...
最新仕様の代表的なサンプル 17 本を、コード・出力・メリット・デメリットとともに解説し、
そのうち 15 本が\textbf{三つの独立な処理系で出力一致}することを確認した
（\texttt{s11} と \texttt{s16} は出力にノード名を含むため自動比較から外し、内容を目視で照合した）。
弾かれるべきプログラムは 28 本になった。
\end{abstract}
...
後者は\textbf{28 本}の「弾かれるべきプログラム」からなる。
...
サンプル 17 本のうち\textbf{15 本が三実装で出力一致}する
（\texttt{s11} と \texttt{s16} は出力にノード名 \texttt{rt0/servo1} を含み、
比較スクリプトが区切りに使う \texttt{/} で割れてしまうため自動比較から外した。
内容は目視で一致を確認している）。
ガイドの 8 本も一致する。
弾かれるべき 28 本については、84 枠のうち\textbf{10 枠}が期待と食い違い、
その内訳は JS 版 7・Py 版 3 である（\textbf{OCaml 版は 28 本すべてで期待どおりに振る舞う}）。
...
現状は、\textbf{機械検証の代わりに「弾かれるべきプログラム」28 本}が
実装の規律を押さえている。
...
\begin{enumerate}
\item \textbf{セッション型}。
      経路 3 は期限と送り手の存在検査で部分的に押さえたが、
      「この順序でやり取りする」という約束は書けない。
      資源にも全体の順序を課す必要がある。
