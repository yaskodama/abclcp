...
後者は\textbf{26 本}の「弾かれるべきプログラム」からなる。
...
サンプル 16 本のうち\textbf{14 本が三実装で出力一致}する
（\texttt{s11} と \texttt{s16} は出力にノード名 \texttt{rt0/servo1} を含み、
比較スクリプトが区切りに使う \texttt{/} で割れてしまうため自動比較から外した。
内容は目視で一致を確認している）。
ガイドの 8 本も一致する。
弾かれるべき 26 本については、78 枠のうち\textbf{10 枠}が期待と食い違い、
その内訳は JS 版 7・Py 版 3 である（\textbf{OCaml 版は 26 本すべてを弾く}）。
...
現状は、\textbf{機械検証の代わりに「弾かれるべきプログラム」26 本}が
実装の規律を押さえている。
...
\begin{enumerate}
\item \textbf{経路 3 ---- \texttt{select} の待ち}。
      来ないメッセージを待つ場合は、レベルでは扱えない。
      セッション型・プロトコル検査の領域である。
      資源にも全体の順序を課す必要がある。
