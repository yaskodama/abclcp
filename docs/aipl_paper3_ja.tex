...
前二つに対して \texttt{reply} の全域性と義務レベルを入れた。
レベルは注釈でも書けるし、\textbf{書かなければ推論される}。
その結果、\textbf{待ちの宛先が静的に分かる範囲では
デッドロック自由が型で保証される}。
...
\paragraph{到達点。}
\textbf{待ちの宛先が静的に分かる範囲では、デッドロック自由が型で保証される。}
注釈は要らない ---- 推論が埋める。
...
後者は\textbf{25 本}の「弾かれるべきプログラム」からなる。
...
ガイドの 8 本も一致する。
弾かれるべき 25 本については、75 枠のうち\textbf{10 枠}が期待と食い違い、
その内訳は JS 版 7・Py 版 3 である（\textbf{OCaml 版は 25 本すべてを弾く}）。
...
現状は、\textbf{機械検証の代わりに「弾かれるべきプログラム」25 本}が
実装の規律を押さえている。
...
\begin{enumerate}
\item \textbf{レベルをアクター型へ載せる}。
      明示宣言と推論は入れた（\S\ref{sec:dl3}）。
      残るのは、レベルをアクター型に載せて
      \textbf{動的な宛先と配備先まで}届かせることである。
      資源にも全体の順序を課す。
