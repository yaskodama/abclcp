...
前二つに対して \texttt{reply} の全域性と義務レベル（\texttt{@n} 注釈）を入れた。
その結果、\textbf{待ちの宛先が静的に分かり、レベル注釈が付いている範囲では
デッドロック自由が型で保証される}。
成立の条件と、依然として残る範囲を明記する。

採否の理由はすべて動く最小プログラムで示す。
最新仕様の代表的なサンプル 15 本を、コード・出力・メリット・デメリットとともに解説し、
そのうち 14 本が\textbf{三つの独立な処理系で出力一致}することを確認した。
弾かれるべきプログラムは 24 本になった。
\end{abstract}
...
3. \texttt{select} の待ち & 難しい & セッション型・プロトコル検査 \\
\bottomrule
\end{tabular}
\end{center}
...
これも既存コードへの影響は 0 本である（\texttt{@} を使うコードが無いため）。
...
\begin{itemize}
\item \textbf{待ちの宛先が静的に分かる範囲に限る}。
      \texttt{now x.m()} の \texttt{x} が引数や \texttt{deploy} 由来だと、
      レベルを\textbf{アクター型に載せる}必要がある（\texttt{actor[C]@$\ell$}）。
      これは全アクター参照の型に波及する。
\item \textbf{精度が落ちる}。レベルはメソッド単位なので、
      同じクラスの別インスタンス同士（親が子を待つ木構造など）も一律に弾かれる。
      緩めるにはレベル変数が要る。
\item \textbf{配備先は見えない}。
      ただし \texttt{node\_allow} が効果を国境で照合しているのと同じ形で、
      レベルも配備の契約に載せられる。
\item \textbf{停止性・飢餓は別問題}。
      \texttt{while 1>0 do \{\}} がメールボックスを塞ぐのはレベルでは止まらない。
\item 経路 3（\texttt{select}）は手つかずである。
\end{itemize}

\paragraph{到達点。}
\textbf{待ちの宛先が静的に分かり、全メソッドにレベル注釈が付いている範囲では、
デッドロック自由が型で保証される。}
...
また注釈は\textbf{人が付ける}。付け忘れた組は検査されない ----
効果注釈と同じ弱点である。

\subsection{15 本を通して見えること}
...
後者は\textbf{24 本}の「弾かれるべきプログラム」からなる。
...
サンプル 15 本のうち\textbf{14 本が三実装で出力一致}する
（残る 1 本 \texttt{s11} は配備の出力にノード名を含むため比較の対象から外した）。
ガイドの 8 本も一致する。
弾かれるべき 24 本については、72 枠のうち\textbf{10 枠}が期待と食い違い、
その内訳は JS 版 7・Py 版 3 である（\textbf{OCaml 版は 24 本すべてを弾く}）。
...
現状は、\textbf{機械検証の代わりに「弾かれるべきプログラム」24 本}が
実装の規律を押さえている。
...
\begin{enumerate}
\item \textbf{レベルの推論とアクター型への搭載}。
      明示宣言は入れた（\S\ref{sec:dl3}）。次は
      呼び出しグラフが非巡回なら自動で番号を振ること、
      そしてレベルをアクター型に載せて動的な宛先と配備先まで届かせること。
      資源にも全体の順序を課す。
