...
compiler \texttt{cc} \textbf{JITs that to AArch64 machine code}. In that
compiler \texttt{int} is 8 bytes, matching AIPL's tagged 64-bit value.

\begin{term}
# POST the AIPL source itself (a body starting with "class " goes through abcl2c)
...
\textbf{Same model, same generated text.}
\texttt{ai\_call("Once upon a time")} agreed to the byte (72 of them) across the
Pi 3, the Pi 5 and the reference build on the Mac. Only the speed differs: the
Pi 5 takes 0.42--0.64 seconds per call. The Pi 3 took 2--6 seconds when freshly
booted, and 16.6 seconds once thirteen modules had been loaded and their actors
had piled up (generation yields to the scheduler once per token, so more
resident actors cost more --- though that causal link has \textbf{not been
isolated}).

The Pi 5's \texttt{abcl2c} accepts \textbf{only the builtins it knows}. Any call
other than \texttt{print} / \texttt{puts} / \texttt{ai\_call} fails with
\texttt{unknown function: <name>}; a misspelled \texttt{ai\_cal("hi")} stops on
the board rather than quietly becoming something else.
