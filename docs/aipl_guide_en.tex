...
\texttt{print} on the Pi 3 goes to \textbf{the serial line}, so it cannot be
read over the network. At present the only way to read a value back from the
hardware is \textbf{external exposure} (\S\ref{sec:expose}).
...
This bites silently. Routes registered earlier \textbf{stay in the table};
only the actors behind them stop working --- calling one gets no reply, and
\texttt{vm\_web\_call} sits out its full 90 seconds and returns empty. After
...
\item \textbf{Try one module at a time} on the hardware; check it before
  loading the next.
\item A stale entry in \texttt{/api/routes} is not evidence that it is alive,
  and the same path can appear twice --- the lookup \textbf{stops at the first
  match}, so the earlier registration wins.
\end{itemize}
...
onto the Pi 3, and starts as an actor (all ten report \texttt{accepted=1}).
That claim holds \textbf{for one module loaded at a time}; it does \textbf{not}
mean the ten run side by side (\S\ref{sec:pi3-onemodule}).
And \texttt{accepted=1} says ``loaded and started'', \textbf{not} that the
output is correct. Values can only be compared for externally exposed paths, so
the samples whose output has been checked on the hardware itself are g5
(\texttt{/echo}) and the \texttt{ai\_call} example (\texttt{/sage}). Output for
the rest is compared on the host VM.
...
Front end & \texttt{compile\_avm} on the Mac & \texttt{abcl2c} \textbf{on the board} \\
Loading & \texttt{POST /actor/loadvm?ask=0} (port 8080) & \texttt{POST /cc} (port 80) \\
Reading values & \texttt{web\_expose} + \texttt{/api/x/} & straight out of the \texttt{/cc} reply \\
Modules resident & \textbf{exactly one} (\S\ref{sec:pi3-onemodule}) & JITed per run \\
Value kinds & integers and strings & integers, strings, floats, lists \\
\texttt{ai\_call} & instruction \texttt{0x52} & runtime \texttt{v\_ai\_call} \\
Kernel & 1.09\,MB $\to$ 2.16\,MB & 0.94\,MB $\to$ 2.02\,MB \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection*{Running something}

Because the front end lives \textbf{on the board}, AIPL source can be sent as
is. A body beginning with \texttt{class} goes through \texttt{abcl2c};
anything else is treated as C.
...
The trailing \texttt{=> 0} is what \texttt{main} returned. From the shell,
\texttt{cc <file>} and \texttt{make <file>} handle \texttt{.abcl} /
\texttt{.aipl} transparently, and \texttt{llm <prompt>} drives the on-board
model directly (its output goes to the serial line).

\subsubsection*{What a program becomes}

What you write stays AIPL, but the generated C is readable. Classes become
\textbf{numbers}, objects are entries in a \texttt{g\_obj[]} array, methods are
functions named \texttt{m\_<Class>\_<method>}, and a method call becomes a
\texttt{dispatch} indexed by those numbers.
...
\texttt{cc/ccpriv.h}). That matches AIPL's tagged 64-bit value, so a string
(a pointer into the heap) held in an \texttt{int} variable is not truncated.

Values are handled through runtime functions: \texttt{v\_int} \texttt{v\_str}
...
\texttt{v\_add} \textbf{concatenates} when either side is a string, so on the
Pi 5 a single \texttt{+} covers both numeric addition and concatenation
(there is no \texttt{++}).

\subsubsection*{The on-board model}

\texttt{ai\_call} lowers to \texttt{v\_ai\_call}, which renders the prompt to a
string, calls \texttt{llm\_run}, and copies the result into the value heap as a
string. The model is \textbf{the same one} as on the Pi 3
(\S\ref{sec:xinu-ai}), and \texttt{ai\_call("Once upon a time")} agreed to the
byte (72 of them) across the Pi 3, the Pi 5 and the reference build on the Mac.
Only the speed differs: \textbf{the Pi 5 takes 0.42--0.64 seconds per call}.
The Pi 3 took 2--6 seconds freshly booted, and 16.6 seconds once thirteen
modules had been loaded and their actors had piled up (generation yields to the
scheduler once per token, so more resident actors cost more --- though that
causal link has \textbf{not been isolated}).

\subsubsection*{What the on-board front end accepts}

\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
Accepted & classes, fields, methods, \texttt{var}, assignment, \texttt{if}/\texttt{else}, \\
     & \texttt{while} (with or without \texttt{do}), \texttt{return}, \\
     & \texttt{new} (arguments allowed when the class has \texttt{init}), \\
     & \texttt{now} (returns a value), \texttt{send}, integers, strings, floats, \\
     & \texttt{+ - * /}, comparisons, \texttt{print} / \texttt{puts}, \texttt{ai\_call} \\
\midrule
Rejected & \textbf{type, level and effect annotations} (\texttt{: string} gives \texttt{method: expected '\{'}) \\
     & \texttt{reply} / \texttt{replyto} / \texttt{future} / \texttt{await} \\
     & \texttt{select} / \texttt{case} / \texttt{timeout} \\
     & \texttt{++} (use \texttt{+}), arrays \texttt{array\_*}, \texttt{spawn}, \texttt{remote} \\
     & any call other than \texttt{print} / \texttt{puts} / \texttt{ai\_call} \\
\bottomrule
\end{tabular}
\end{center}

Refusals name the thing: \texttt{not supported on this device: future},
\texttt{unknown function: sqrt},
\texttt{new: class has no init to take arguments}.
\textbf{Nothing quietly turns into something else.}

\textbf{This is a good deal narrower than the canonical language.} None of
g1--g10 from \S\ref{part:sample} passes this front end, because every one of
them carries a return-type annotation. To run them on the Pi 5, drop the
annotations, replace \texttt{reply} with \texttt{return} plus \texttt{now}, and
turn \texttt{++} into \texttt{+}. The Pi 3's \texttt{.avm} path
(\S\ref{sec:xinu}) \textbf{accepts and discards} annotations, so canonical
sources pass there unchanged --- the two boards' front ends have different
temperaments.
