...
\section{Running on real hardware --- Xinu / Raspberry Pi 3, Pi 4 and Pi 5}
\label{sec:xinu}

AIPL programs also run on \textbf{bare-metal Raspberry Pis} (Embedded Xinu):
no operating system underneath, no libc, and the kernel itself carries the
actor machinery. There are \textbf{three boards}. All of them accept the same
canonical AIPL, but the machinery inside differs on each --- which is much of
what makes this section worth reading.
...
\toprule
 & Pi 3 & Pi 4 & Pi 5 \\
\midrule
Address & \texttt{192.168.3.50:8080} & \texttt{192.168.3.100:80} & \texttt{192.168.3.101:80} \\
Execution & a VM interprets \texttt{.avm} & \multicolumn{2}{c}{AIPL$\to$C$\to$in-kernel \texttt{cc} JIT} \\
Front end lives & on the Mac & \multicolumn{2}{c}{\textbf{on the board} (\texttt{abcl2c})} \\
Actor vehicle & \multicolumn{2}{c}{\textbf{real Xinu processes}} & cooperative pump (one thread) \\
Ten guide samples & 9 match & \textbf{all ten} & \textbf{all ten} \\
\bottomrule
\end{tabular}
\end{center}

The Pi 4 sits \textbf{between the other two}: it executes like the Pi 5 (JIT)
and schedules like the Pi 3 (real processes).
...
\midrule
Classes, fields, \texttt{send} / \texttt{send!} & yes & \texttt{send!} is treated as \texttt{send} \\
Method-local \texttt{var}, \texttt{reply} & yes & locals become hidden fields \\
\texttt{if} / \texttt{while} & yes & parentheses optional \\
...
\ty{float} and float literals & yes & boxed in a table; the tag is three bits wide \\
Arrays (\texttt{array\_*}) & yes & \texttt{push}/\texttt{set} return a new array, canonically \\
Math builtins (\texttt{sqrt} etc.) & yes & the same series as the Pi 4 and Pi 5 (\texttt{sqrt} by Newton) \\
The \texttt{call} statement (a builtin called as a statement) & yes & checked on hardware \\
Deadline without \texttt{else} (\ty{result}) & yes & failure is a reserved value; no new instruction \\
\texttt{is\_ok} / \texttt{value} & yes & lowered to a comparison and a branch \\
\texttt{replyto} / \texttt{answer} / parameter \texttt{reply} & yes & a reply destination is an actor id (\texttt{\_\_snd}) \\
\texttt{acquire} / \texttt{release} & yes & new instructions \texttt{0x53}/\texttt{0x54}; named locks \\
\texttt{spawn} & yes & the new actor id goes into the published table under that name \\
\texttt{remote(...)} sends & yes & instructions \texttt{0x55}/\texttt{0x56}; carried over UDP/9010 \\
\texttt{await} in expression position & no & only \texttt{var x = await f;} (continuation splitting) \\
\bottomrule
\end{tabular}
\end{center}
...
g8  & \texttt{... / b = true / not-eq: true} & matches \\
...
The one mismatch is not a gap in the front end; it comes from the shape of the
execution vehicle. (g8's \texttt{b = true} was brought into line on both boards on
2026-09-04, \S\ref{sec:bool-tag}.)
...
\midrule
Execution & a VM interprets \texttt{.avm} & \multicolumn{2}{c}{AIPL$\to$C$\to$in-kernel \texttt{cc} JIT} \\
Front end & \texttt{compile\_avm} (Mac) & \multicolumn{2}{c}{\texttt{abcl2c} on the board (same source)} \\
Submission & \texttt{POST /actor/loadvm?ask=0} & \multicolumn{2}{c}{\texttt{POST /cc}} \\
Port & 8080 & 80 & 80 \\
Reading output & \texttt{/api/console} & \multicolumn{2}{c}{in the reply} \\
Reading return values & \multicolumn{3}{c}{\texttt{web\_expose} + \texttt{/api/x/} (all three)} \\
Programs resident & \multicolumn{3}{c}{one (a new one replaces it)} \\
Actor vehicle & \multicolumn{2}{c}{\textbf{real Xinu processes}} & cooperative pump (one thread) \\
\texttt{select} & \multicolumn{3}{c}{continuation split + interception (all three)} \\
\texttt{future} / \texttt{await} & continuation split & \multicolumn{2}{c}{runs at once} \quad (none concurrent) \\
\texttt{wait(ms)} & a VM opcode & on the app worker & on a dedicated process \\
Booleans & \multicolumn{3}{c}{a tagged value; printed \texttt{true} / \texttt{false} (\S\ref{sec:bool-tag})} \\
Value kinds & integers and strings & \multicolumn{2}{c}{int, string, float, list} \\
\texttt{ai\_call} & opcode \texttt{0x52} & \multicolumn{2}{c}{runtime \texttt{v\_ai\_call}} \\
\quad measured & 2--6 s & 0.79 s & \textbf{0.42 s} \\
Ten guide samples & \textbf{9 match} & \textbf{all ten} & \textbf{all ten} \\
Kernel & 1.09$\to$2.16\,MB & 2.02$\to$2.07\,MB & 0.94$\to$2.05\,MB \\
\bottomrule
\end{tabular}
\end{center}
...
Accepted & classes, fields, methods, \texttt{var}, assignment,
     ...
     & \textbf{\texttt{send!}} (treated as \texttt{send}), \textbf{\texttt{call g(args);}}
       and bare \texttt{g(args);} (calling one's own method) \\
     & \textbf{Floats} --- literals and \texttt{float} fields, passed to the value
       runtime's \texttt{v\_floatlit} as an IEEE-754 bit pattern \\
     & \textbf{Arrays} \texttt{array\_empty} / \texttt{push} / \texttt{get} /
       \texttt{set} / \texttt{len} / \texttt{zeros} --- mapped onto the runtime's lists \\
     & \textbf{Maths builtins} \texttt{sqrt exp log log10 sin cos tan asin acos
       atan floor ceil round abs neg} --- written by hand, since there is no
       \texttt{libm} (\texttt{sqrt} is the \texttt{fsqrt} instruction; the rest
       reduce the range and then use a series, \S\ref{sec:hw-math}) \\
     & \textbf{\ty{result}} --- a deadline written without \texttt{else}
       (\texttt{now o.m() timeout N}, \texttt{await f timeout N}) plus
       \texttt{is\_ok(r)} / \texttt{value(r, default)} \\
     ...
     & \textbf{\texttt{replyto} / \texttt{answer} / parameter \texttt{reply}}
       --- since \texttt{reply(v)} is the return value here, one reply slot is
       taken per \texttt{now} to carry a destination as a value \\
     & \textbf{\texttt{acquire} / \texttt{release}} --- pairing and order are
       checked canonically; what is kept here is a runtime watch only \\
     & \textbf{\texttt{remote("host:port","actor")}} --- send to, or call, an
       actor another node published. UDP/9010; the datagram is shared by all
       three boards and the Mac host VM \\
     & a field initialiser may be \texttt{new K()} (the canonical way to hold
       an actor is in a field, so without this you get
       \texttt{send target is not actor}) \\
...
g8 & \texttt{literal true ok} / \texttt{literal false ok} / \texttt{b = true} & yes \\
...
\textbf{g8's \texttt{b = true} was brought into line on 2026-09-04}
(\S\ref{sec:bool-tag}). Until then both boards held \ty{bool} as an \ty{int} and
printed \texttt{b = 1}.
...
\subsubsection*{The maths builtins are written by hand}
\label{sec:hw-math}

There is no \texttt{libm} on bare metal, so the canonical \texttt{Core.Math}
fifteen live inside the value runtime. \texttt{sqrt} is AArch64's \texttt{fsqrt}
instruction; the rest reduce the range and then evaluate a series.

\textbf{Accuracy was checked against \texttt{libm} on the Mac before shipping.}
The first version was off by $2.4\times10^{-4}$ for \texttt{sin} and by
$10^{-2}$ for \texttt{atan}/\texttt{asin}/\texttt{acos}: the range reduction was
too weak and the series had not converged. Folding \texttt{sin}/\texttt{cos} to
multiples of $\pi/2$ and picking by quadrant, and shrinking \texttt{atan} below
$\tan(\pi/12)$, fixed it.
...
\begin{tabular}{@{}ll@{}}
\toprule
Builtin & Worst relative error vs.\ \texttt{libm} \\
\midrule
\texttt{sqrt} / \texttt{floor} / \texttt{ceil} / \texttt{round} & \textbf{0} (exact) \\
...
\texttt{abs} and \texttt{neg} return an integer when given one (the canonical
implementation only accepts \ty{float}, so the hardware is the laxer of the two).

\subsubsection*{\ty{result} --- a deadline without \texttt{else}}

Writing \texttt{now o.m() timeout N} or \texttt{await f timeout N} without an
\texttt{else} yields \ty{result}$\langle\tau\rangle$. On the hardware this is
\textbf{the value itself on success} and \textbf{one reserved value on failure} ---
success is not wrapped, so nothing is allocated. \texttt{is\_ok(r)} and
\texttt{value(r, default)} are the only observers, which is all that is needed.
...
The failure value prints as \texttt{err} and is false as a condition.

\subsubsection*{Booleans are a tagged value}
\label{sec:bool-tag}

The canonical implementation prints \texttt{print("b = " ++ b)} as
\texttt{b = true}. Both boards printed \textbf{\texttt{b = 1}} for a long time:
booleans were plain 0/1, and the value carried nothing that would print them back
as \texttt{true} / \texttt{false}. \textbf{Both were fixed together on 2026-09-04}
--- fixing one alone would have re-opened a difference we had just closed.

The scheme is the same tagged-value trick used for strings; only where the tag
lives differs.
...
\begin{tabular}{@{}lll@{}}
\toprule
 & Pi 3 (\texttt{.avm} VM, 32-bit) & Pi 5 (JIT, 64-bit) \\
\midrule
Tag & \texttt{0x20000000} (bit 29) & bit 62 (low bit 0) \\
Value & the low bit & bit 1 \\
Strings & \texttt{0x40000000} (bit 30) & a real address ($< 2^{48}$) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{On the Pi 5 the tag cannot live on the integer side.} \texttt{v\_int(-1)}
has every high bit set, so a high-bit tag would \textbf{turn negative numbers into
booleans}. It goes in the ``low bit 0, i.e. not an integer'' space instead
(floats use bit 60, lists bit 61).

\textbf{Unlike strings, a boolean's tag must also be read when testing a
condition.} \texttt{false} carries the tag, so it is \textbf{not zero}, and a plain
branch-if-zero would not see it as false. The Pi 3 routes \texttt{JZ}
(\texttt{0x31}) through \texttt{vm\_falsy}; on the Pi 5 \texttt{v\_truthy} checks for
a boolean first. Comparisons and logical operators
(\texttt{< <= > >= == != \&\& || !}) now return booleans.
...
\textbf{One path still prints \texttt{1}}: the Mac-side \texttt{--xinu-jit}
(\texttt{POST /actor/load}) collapses \texttt{true} / \texttt{false} to
\texttt{Int 1} / \texttt{Int 0} in its front end. Fixing that means adding a boolean
to the AST, which reaches into type inference.
...
\subsection{Three mesh verbs --- addressing nobody}
\label{sec:xinu-mesh}

\texttt{remote} writes one address and calls it synchronously. That is fine when
the peer is fixed. A mesh is different: nodes move, routes change, and nothing
guarantees that everyone answers. \textbf{Not being able to write the address
down is the point.}

So three verbs were added, under the effect \texttt{mesh}:

\begin{lstlisting}
neighbors() : string[]                  // IPs of the nodes answering right now
broadcast(svc, method, arg) : unit      // send to all; expect no reply
gather(svc, method, arg, ms) : int[]    // ask all; keep what arrives in time
\end{lstlisting}
...
class Probe {
  ...
  method survey() : unit !{mesh, log} {
    ...
    broadcast("probe", "count", 1);              // nothing comes back
    var rs = gather("probe", "count", 2, 1500);  // only what arrives
...
\textbf{No address appears anywhere.} That is the decisive difference from
\texttt{remote}: when routes change or a node disappears, the program does not.

\subsubsection*{Why \texttt{gather} returns an array}

\textbf{The array in the return type is itself the specification.} In a mesh,
partial success is the normal case, so a type that assumes everyone answers
would be a lie. A node that did not answer simply is not in the array --- no
new failure machinery was introduced (\texttt{result} and \texttt{timed\_out}
are unchanged). A node that lacks the actor replies \texttt{err}, and that is
dropped too: \textbf{"did not arrive" and "did not have it" are deliberately
indistinguishable in the array.}

\subsubsection*{Why \texttt{mesh} is separate from \texttt{net}}

\texttt{net} says only "this leaves the machine". \texttt{mesh} says, in
addition, that \textbf{the route is dynamic and may break mid-flight}. Keeping
them apart lets \texttt{node\_allow(...)} say "\texttt{net} yes, \texttt{mesh}
no" at a node border. The check bites:

\begin{term}
method C.m declares {log} but its body has {log, mesh} (missing {mesh})
\end{term}

\subsubsection*{The wire}

The same UDP port 9010 as \texttt{remote}. No peer table: the message goes to
the subnet broadcast (\texttt{192.168.3.255} / \texttt{ff:ff:ff:ff:ff:ff}).

\begin{term}
H <id>                          anyone there?  (broadcast)
A <id>                          yes  (the sender's IP is the answer)
B <id> <svc> <method> <arg>     fire and forget; no reply
Q <id> <svc> <method> <arg>     ask everyone; replies come back as R
\end{term}

\texttt{broadcast} got its own verb \texttt{B} for a reason. The first attempt
reused "\texttt{Q} with reqid 0", but then the at-most-once cache collapses to
a single \texttt{(sender, 0)} entry and \textbf{every later broadcast is
silently dropped}. A separate verb keeps the cache usable as it is.

\subsubsection*{Measured on the boards}

Four nodes (Pi 3, Pi 4, Pi 5 and the Mac host VM) on wired Ethernet. From each
node, \texttt{gather} collects the other three, every time.
...
That \texttt{broadcast} works is confirmed not by counting replies but by
\textbf{watching the value move}: no reply arrives, yet the peer's
\texttt{seen} goes $2 \to 12$. Repeated broadcasts never make it $22$, which
confirms the at-most-once cache at the same time.

\subsection{\texttt{remote} --- calling an actor on another board}
\label{sec:xinu-remote}

Canonically a send target is either a variable holding an actor reference or
\texttt{remote("host:port", "actorName")} (see the table in
\S\ref{sec:reply1st}). The second form now works on all three boards and on the
Mac host VM. What it names is an actor published with \texttt{web\_expose}.
...
\subsubsection*{The datagram}

All four implementations speak the same language: UDP port 9010, one line of
readable ASCII (so \texttt{tcpdump} and \texttt{nc} show it as it is).
...
\begin{itemize}
\item What travels is the method \textbf{name}, not its number: numbers differ
  per board, so the receiving board resolves the name with its own
  \texttt{\_\_method\_id}.
\item One argument. \texttt{<arg...>} runs to the end of the line, so it may
  contain spaces. A string of digits arrives \textbf{as an integer} on the other
  side (the canonical \texttt{now remote(...).step(3)} takes one).
\item What comes back is the value re-read from the text the other side
  rendered. \textbf{No type is carried} --- the promise holds because the
  canonical type checker has already matched the types on both sides.
\item A deadline, an unknown host and an unknown method are \textbf{all}
  failures. With \texttt{else} you get that value; without it you get
  \texttt{err} (\ty{result}). The timeout case stays in one place.
\end{itemize}

\subsubsection*{Retransmission, and at-most-once}

\textbf{UDP drops packets, so the request is re-sent every 200\,ms.} While it
was sent once and then waited for, a board whose wait loop turns only a few
times a second would \textbf{always} time out.

Retransmitting naively runs the remote method twice, so the receiver keeps the
last answer per \texttt{(sender IP, reqid)} and, for a repeat of the same
request, \textbf{returns the stored answer without running anything}. That makes
it \textbf{at most once}, which is safe even for a method with side effects.

\subsubsection*{How each board carries it}
...
Board & Carriage \\
\midrule
Pi 3 & the Xinu UDP device layer (\texttt{udpAlloc} / \texttt{open} /
       \texttt{read} / \texttt{write}); the receiving end is a resident process.
       VM instructions \texttt{0x55} / \texttt{0x56} \\
Pi 4 / Pi 5 & their TCP is listen-only (passive open), so the frames are built
       by hand \\
Mac & OCaml Unix sockets. It is a \textbf{fourth peer}, which is what lets you
       flash one board at a time and still have something to talk to \\
\bottomrule
\end{tabular}
\end{center}

The Pi 4 and the Pi 5 \textbf{never resolve the destination MAC with ARP}. The
first request goes out with a broadcast destination MAC and a unicast
destination IP, and \textbf{the decision to take it is made by the receiver}.
The reply goes back to the requesting frame's sender, so no ARP is needed, and
the MAC learned there is remembered --- later requests are unicast. That breaks
less than another ARP state machine, and is enough for a handful of boards.

\subsubsection*{Registering the callee (this differs per board)}

The actor a call names must be \textbf{resident} on the board.

\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
Pi 4 & post it with \texttt{POST /cc?resident=1} \\
Pi 5 & a plain \texttt{POST /cc} is enough \\
Pi 3 & whatever was sent to \texttt{/actor/loadvm?ask=0} stays \\
\bottomrule
\end{tabular}
\end{center}

A plain \texttt{POST /cc} on the Pi 4 runs the program and is done, so
\textbf{nothing is left for an outside caller to reach} and the caller gets
\texttt{err}.

\subsubsection*{Measured}

That \texttt{say(21)} answers \texttt{42} was checked for \textbf{every pair} of
...
\toprule
Caller $\rightarrow$ & Pi 3 & Pi 4 & Pi 5 & Mac & unreachable \\
\midrule
...
\subsection{Traps common to all three boards (implementation side)}
\label{sec:three-boards-traps}

These are invisible to someone writing AIPL, but they are the holes we fell
into \textbf{on two or more boards}. Putting actors on bare metal seems to trip
in the same places every time.

\begin{itemize}
\item \textbf{Never take an actor thread down with \texttt{kill()}.}
  An actor can be inside an \textbf{interrupt-masked region} (allocating from
  the value heap, posting to a mailbox) or \textbf{holding a lock}. Killing it
  there leaves interrupts masked or the lock held: the board dies outright, or
  everything that waits on that lock spins forever. The right way is to
  \textbf{mark it and wake it}, and let it leave its own loop.
  \textbf{We hit this independently on the Pi 3 and the Pi 4.} The prerequisite
  is putting the ``you are dead'' flag into the \emph{waiting} loop's condition
  --- without that, killing looks like the only option.
\item \textbf{Never leave a pointer into code you freed.} When one program
  replaces another, a function pointer left over from the previous run points
  into freed memory; the next allocation hands that memory out again, so the
  stale pointer \textbf{writes into someone else's buffer}. On the Pi 4 this
  surfaced as \textbf{a single corrupted byte}: a fixed offset of the next
  translation turned \texttt{'i'} into \texttt{'j'}, giving either
  \texttt{cc: undefined variable} or an instant board death.
  \textbf{Because the symptom depends on where the byte lands, it looks like a
  different bug every time.}
\item \textbf{Never yield to actors while holding the value-heap lock.} An
  actor takes the same lock before its handler, so whatever you yielded to
  spins. \textbf{If the lock spins and ``steals'' after a limit, small examples
  happen to work} --- the nastiest shape a bug can take.
\item \textbf{If you put request/response on UDP, retransmission is part of the
  design.} Sent once and waited for, a request will always time out as soon as
  the receive side lags even slightly. On bare metal this looks like a broken
  receive path, and I rebuilt that path \textbf{twice}, each time only thinning
  the symptom. \textbf{The test that settles it is to have the peer answer
  repeatedly} --- if that works, what is broken is not the receive path but the
  missing retransmission. And once you retransmit, keep a
  \texttt{(sender, reqid)} record on the receiving side so it stays
  \textbf{at most once}.
\item \textbf{``I poked it while it was waiting and got an answer'' does not
  prove the receive path is live.} It can be a late answer that your own wait
  window happened to catch. \textbf{Look at the time, not at whether it came.}
  Make the peer log when it sends, and keep two separate counts on the board:
  how many answers arrived, and how many were still being waited for. Misreading
  this cost a round trip before the real cause (the receive window in
  \S\ref{sec:xinu-remote}) came out.
\item \textbf{After flashing, an HTTP answer is not proof that the board
  rebooted.} Until the power is cycled the board keeps answering from the old
  kernel, and polling cannot tell the difference. The symptom is ``the thing I
  just fixed is not there'', which sends you back to doubt the code. All three
  boards now report their build from \texttt{GET /version} (\texttt{kversion.c}
  is rebuilt every time through a \texttt{FORCE} dependency --- without that
  \verb|__DATE__| freezes and the version lies).
\item \textbf{Do not guess at a cause after the board dies; make the failure
  observable first.} A dead board yields no information and every experiment
  costs a power cycle. \textbf{One flash spent on instruments beats five spent
  guessing.} On the Pi 4 the decisive step was adding a ``translate only''
  stage (\texttt{?stage=xlat}), which tells you whether the board is healthy
  without running anything.
\end{itemize}

\subsection{The Pi 4 --- the third board}
\label{sec:xinu-pi4}

The Pi 4 (\texttt{192.168.3.100}) sits \textbf{between the other two}: it
executes like the Pi 5 (AIPL lowered to C on the board and JITed) but its
\textbf{actors are real Xinu processes} like the Pi 3's
(\texttt{system/actorproc.c}). The on-board front end \texttt{abcl2c} is
\textbf{the same source} as the Pi 5's, and for all ten canonical guide samples
its output is \textbf{byte-identical} to the host translation.
...
\texttt{/cc} has \textbf{stages}. They let you check the front end without
running anything, which is where to start when touching the hardware.

\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
\texttt{POST /cc} & translate and JIT (default) \\
\texttt{POST /cc?stage=xlat} & \textbf{return the translated C; run nothing} \\
\texttt{POST /cc?resident=1} & resident load (reachable later via \texttt{/api/x/}) \\
\texttt{POST /compile} & treat the body as plain C and JIT it \\
\texttt{POST /actor/load} & resident load of Mac-side \texttt{--xinu-jit} C \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Health is measurable with \texttt{?stage=xlat}}: compare the board's
translation with the host's, byte for byte. It is the one window that does not
risk the board.
...
\subsubsection*{The ten canonical guide samples on hardware}

\textbf{All ten behave canonically} (raw AIPL to \texttt{POST /cc}, run back to back).

\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
Sample & Output on the hardware (\texttt{POST /cc}) \\
\midrule
...
g5  & exposed, then \texttt{echo: hi} from \texttt{/api/x/echo} \\
...
The translation is still byte-identical to the host's after the whole run.
Regressions pass too: plain C through \texttt{/compile}, \texttt{/actor/load},
\texttt{ai\_call} producing the same reference text as the Pi 3 and Pi 5
(\textbf{0.79 s}), and \texttt{/smp-bench} at 3.00$\times$ on four cores.

\subsubsection*{\texttt{wait} simply works}

On the Pi 5, \texttt{/cc} had to be moved onto a dedicated process before
\texttt{wait} could sleep. \textbf{The Pi 4 was already built that way}: HTTP is
served by a \textbf{dedicated app worker}, with an \textbf{AIPL/LLM runtime
process} behind it (\texttt{net\_proc\_main} / \texttt{app\_proc\_main} /
\texttt{aipl\_proc\_main} in \texttt{loader/main.c}). So \texttt{wait(300)} is a
real sleep and g7 matches at \texttt{slow(timed out) = 0} --- no need to pick a path.

\subsubsection*{What to read when it stalls}

The Pi 4 carries a full set of instruments. \textbf{Read them while the board is
still alive.}
...
\begin{tabular}{@{}ll@{}}
\toprule
\texttt{GET /fault} & the last CPU fault (ESR / FAR / ELR / SPSR / SP) \\
\texttt{GET /api/aptab} & the raw actor table (pid, process state, queue, dead) \\
\texttt{GET /api/aprun} & \texttt{ap\_run} give-ups, forced kills, worker replacements \\
\texttt{GET /api/mem} & free total, largest block, fragment count \\
\texttt{GET /jitstats} & \texttt{spawn\_fails}, dropped messages \\
\texttt{GET /ticks} & stack canary (\texttt{stkbad}), context-switch count \\
\texttt{GET /wx} & whether W$^\wedge$X is enforced (it probes by writing) \\
\texttt{GET /reboot} & \textbf{watchdog reset} --- no need to touch the power \\
\bottomrule
\end{tabular}
\end{center}

\textbf{\texttt{/reboot} only works while the board is alive.} Use it before you
run out of board.

\subsubsection*{What being real processes changes}

\begin{itemize}
\item \textbf{g9's ordering can vary between runs.} The target of
  \texttt{send x.ping()} may run before \texttt{print("after send")}. The Pi 3
  shares this property. \textbf{Both are correct AIPL}: \texttt{send} promises
  no ordering.
\item After a top-level \texttt{send}, the board \textbf{lets the target run to
  quiescence} before continuing, to match the Pi 5's cooperative ordering.
  Without that, \texttt{select}'s interception does not fire.
\end{itemize}
...
\begin{itemize}
\item \textbf{A tick count is not a time.} \texttt{timer\_ticks()} was treated
  as 100\,Hz (10\,ms per tick); measurement on Pi 4 / Pi 5 says
  \textbf{$\simeq$2\,ms per tick ($\simeq$500\,Hz)}. A retransmit meant to be
  every 200\,ms went out every 40\,ms, and a window meant to be 1500\,ms closed
  after 240\,ms. The symptom: a peer that answers in 1\,ms is collected, a peer
  that needs 100\,ms is always lost. You cannot see this from inside the board
  --- \textbf{line up the wall-clock times of the packets it sends}
  (\texttt{42.558 / 42.598 / 42.678 \ldots}). The fix is to stop assuming a tick
  rate and derive milliseconds from \texttt{cntpct\_el0} / \texttt{cntfrq\_el0}.
  \textbf{\texttt{remote} carried the same bug}: \texttt{timeout 2000} really
  expired after about 400\,ms.
\item \textbf{Build a slow peer to bisect.} Put a responder on the Mac that
  waits 300\,ms before answering and run it next to a fast one. If only the slow
  one is lost, suspect the window, not the receive path.
\item \textbf{"Only my own IP" drops every broadcast.} All three boards filtered
  incoming packets on destination IP. Pi 4 / Pi 5 just needed a wider test, but
  \textbf{Pi 3's Xinu \texttt{udpDemux} requires the device's local IP to equal
  the destination IP}, so rather than patching the stack a second listener was
  bound to the broadcast address (a blocking read cannot serve both).
\item \textbf{Replies go to the source port of the request.} Pi 3 opened its
  sending device with local port 0 (any), so answers landed on an ephemeral port
  that nothing was reading. \textbf{The other board's instrument said so}: Pi 4's
  \texttt{rx\_q} counted 8 broadcasts from Pi 3 while Pi 3's \texttt{gather}
  stayed at 0 --- it goes out, the return does not line up.
\end{itemize}
...
\begin{itemize}
\item \textbf{What runs on the hardware (Xinu / Pi 3, Pi 4, Pi 5) is a subset of
  the canonical language.} No \ty{float}, arrays, math builtins, \texttt{spawn},
...
\subsection{2026-09-07 --- three mesh verbs}

\texttt{neighbors} / \texttt{broadcast} / \texttt{gather}, which name no
destination, plus the effect \texttt{mesh} (\S\ref{sec:xinu-mesh}). All four
implementations (canonical, host VM, Pi 3, Pi 4 / Pi 5) speak the same wire, so
the canonical is itself the fourth node. Verified on four wired nodes: from each
one, \texttt{gather} collects the other three every time.

Three latent hardware bugs were fixed along the way: a tick count mistaken for a
time (which also made \texttt{remote} deadlines expire five times early), a
missing at-most-once answer cache on Pi 4, and Pi 3's reply-port mismatch.

\label{sec:migrate-en}

\subsection*{2026-09-06 --- the canonical 25 features now run on hardware}

The language did not change. What changed is \textbf{how much of it the boards
accept}.

\begin{itemize}
\item \textbf{\texttt{remote("host:port","actor")}} (\S\ref{sec:xinu-remote}).
  The three boards and the Mac host VM can call each other's published actors.
  \textbf{Checked for every pair.} Carried over UDP port 9010; the datagram is
  shared by all four implementations.
\item \textbf{First-class replies} (\texttt{replyto} / \texttt{answer} /
  parameter type \texttt{reply}). On the Pi 3 a reply destination \emph{is} an
  actor id, so the front end alone was enough; on the Pi 4 and Pi 5
  \texttt{reply(v)} is the return value, so a reply slot is taken per
  \texttt{now}.
\item \textbf{Resource order} (\texttt{acquire} / \texttt{release}). Pairing and
  order stay canonical; the boards keep a runtime watch. On the Pi 3 it is a
  \textbf{real named semaphore} --- actors there are real processes, so two of
  them can genuinely contend.
\item \textbf{\ty{result} with \texttt{is\_ok} / \texttt{value}}, \ty{float},
  arrays and the maths builtins (Pi 4 and Pi 5), and \texttt{new K()} in a field
  initialiser.
\item \textbf{\texttt{spawn("Class","name")}} --- create an actor under a name.
  None of the three keeps a separate registry of named actors: the
  \textbf{published table} (the one \texttt{web\_expose} fills) is that registry,
  so the new actor id goes in under that name and
  \texttt{remote("host","name")} and \texttt{/api/x/name} reach the same thing.
\item \textbf{The Pi 3 gained \ty{float}, arrays and the maths builtins.} Its
  value tag became a \textbf{three-bit pattern} in the top bits rather than a set
  of independent bits, which is what let more kinds of value in without narrowing
  the integer range; reals and arrays are boxed in a table. The maths uses the
  same series as the Pi 4 and Pi 5, so all three print the same thing.
\item \textbf{That completes the canonical 25 on all three boards.}
\end{itemize}

\subsection*{2026-09-05 --- all three boards accept the same AIPL}

The language did not change; the \textbf{hardware coverage} did.

\begin{itemize}
\item \textbf{The Pi 4 was brought up to current AIPL} (\S\ref{sec:xinu-pi4}):
  the on-board front end \texttt{abcl2c} was ported, so raw AIPL can be posted
  to \texttt{POST /cc}. \textbf{All ten canonical guide samples behave canonically.}
\item \textbf{\texttt{select} now has the same shape on all three}
  (\S\ref{sec:pi5-select}). The Pi 5's version did not wait; it was rebuilt on
  the Pi 3's continuation-split scheme.
\item \textbf{\texttt{wait} works under the Pi 5's \texttt{/cc}}
  (\S\ref{sec:pi5-budget}); there is no longer a path to choose.
\item \textbf{Booleans became a tagged value} (\S\ref{sec:bool-tag}): all three
  print \texttt{b = true}. Every board used to print \texttt{b = 1}.
\item \textbf{The Pi 3's output is readable over HTTP} (\S\ref{sec:pi3-console}).
  Adding \texttt{/api/console} is what finally allowed comparing all ten
  outputs on the hardware itself.
\item Fixed across the boards: \texttt{init()} never called for \texttt{new C()},
  a timed-out call's late reply being taken by the next wait, a double
  \texttt{init}, and actors surviving a program swap.
\end{itemize}
