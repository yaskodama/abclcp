...
\texttt{call} / calling one's own method & no & the VM has no call instruction \\
...
\item \textbf{That completes the canonical 25 on hardware.} Only the Pi 3 still
  lacks \ty{float}, arrays, the maths builtins and calling one's own method ---
  all of which follow from \texttt{.avm} values being 32-bit tagged integers
  (22 of 25).
\end{itemize}
