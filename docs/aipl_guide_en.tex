...
\item \textbf{\ty{result} with \texttt{is\_ok} / \texttt{value}}, \ty{float},
  ...
  initialiser.
\item \textbf{Only \texttt{spawn} is left.} The Pi 3 additionally lacks
  \ty{float}, arrays, the maths builtins and calling one's own method --- all of
  which follow from \texttt{.avm} values being 32-bit tagged integers.
\end{itemize}
