...
\section{Running on real hardware --- Xinu / Raspberry Pi 3}
\label{sec:xinu}
...
Note what that claim covers: \texttt{accepted=1} says ``loaded and started'',
\textbf{not} that the output is correct. Values can only be compared for
externally exposed paths, so the samples whose output has been checked on the
hardware itself are g5 (\texttt{/echo}) and the \texttt{ai\_call} example
(\texttt{/sage}). Output for the rest is compared on the host VM.
