...
\texttt{wait(ms)} & a VM opcode; works on every path & \textbf{shell path only} \\
Ten guide samples & \textbf{8 match exactly} & \textbf{8 match exactly} (9 on the shell path) \\
...
\midrule
g7 & \texttt{slow(timed out) = 42} & \textbf{no} (canonical: 0) \\
g4 & \texttt{waiting} / \texttt{timed out} & \textbf{no} (canonical: \texttt{via select:1}) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{g7 does match on the other path.} Under \texttt{/cc}, \texttt{wait(300)}
is unavailable (\S\ref{sec:pi5-budget}), so the call takes nowhere near 300\,ms
and never exceeds the 50\,ms deadline. On the shell path \texttt{wait} is a real
sleep, and the measured result is \texttt{slow(timed out) = 0}.

\begin{term}
curl "http://192.168.3.101/run?cmd=cc%20/home/g7.abcl"
  fast = 42
  slow(timed out) = 0        <- matches the canonical answer
  await = 10
\end{term}

\textbf{g4 started matching on 2026-09-03--04} (\S\ref{sec:pi5-select}), once
\texttt{select} was rebuilt on the Pi 3's continuation scheme.
\textbf{The two boards now cover the same ground.}
...
\begin{tabular}{@{}lll@{}}
\toprule
Path & Budget & \texttt{wait(ms)} \\
\midrule
\texttt{POST /cc}, \texttt{/actor/load} & \textbf{250 ms} & refused \\
shell \texttt{cc} / \texttt{make} & \textbf{10 s} & \textbf{works} \\
\bottomrule
\end{tabular}
\end{center}

The first runs \textbf{inside the interrupt-masked receive path}, so sleeping
there freezes the board. \texttt{wait} is therefore refused, and anything over
250 ms ends with
\texttt{cc: aborted (ran past the runaway-loop deadline)}. The second runs in
\textbf{its own process on its own stack}, so a runaway can only blow that
stack; there, and only there, \texttt{wait} is a real sleep.
...
\quad of which canonical & 0 & \textbf{8} (9 on the shell path) \\
...
\texttt{wait(ms)} & refused & shell path only \\
