...
\section{実行して分かったこと：評価器が型検査器と食い違う}
\label{sec:runtime}
...
\texttt{VFloat} が返る。原因は \texttt{eval\_thread.ml} の
\texttt{apply\_binop} にある。
...
整数どうしであっても \texttt{VFloat} を返している。一方
\texttt{typing\_env.ml} は \texttt{+ : (int * int) -> int} を持っている。
すなわち $\vdash e : \ty{int}$ でありながら $e \Downarrow \ty{VFloat}$ で、
\textbf{preservation が成り立っていない}。
...
直し方は 2 通りあり、どちらを採るかは言語仕様の決定になる。

\begin{enumerate}
\item \texttt{VInt} どうしの \texttt{+ - *} は \texttt{VInt} を返すようにし、
  \texttt{/} は \ty{float} のままとして \texttt{typing\_env} の \texttt{/} を
  $(\ty{int} \times \ty{int}) \to \ty{float}$ に変える。
\item \texttt{/} も含めて整数演算にする。型付けはそのままでよいが、
  \texttt{7 / 2} が \texttt{3} になるので既存プログラムの数値が変わる。
\end{enumerate}

いずれも評価器の意味論を変えるため、本稿では手を入れていない。
再現用のサンプルを付録 A.13 として残した。
...
\item \textbf{評価器が型検査器と食い違っている}（\S\ref{sec:runtime}）。
  \texttt{apply\_binop} が整数どうしの演算でも \texttt{VFloat} を返すため、
  \ty{int} と型付けした式が実行時に float になる。
  意味論の変更を伴うので本稿では直していない。
\end{enumerate}
