...
既知の食い違い 1 件（expect-mismatch 宣言あり）:
  s5_overload_ambiguity.abcl: Calc#add は型検査では float だが、
                              実行時は int を reply した
想定外の食い違いなし
\end{lstlisting}
...
唯一の食い違いは \texttt{s5}、すなわち principal type が無いことを
示すために置いてある負例だけである。\texttt{abclc/now\_future.abcl} も
同じ形だったので、\texttt{: int} を注釈して静的・動的を揃えた。
\textbf{残る食い違いが「型検査器自身が曖昧と警告しているもの」に
一致している}ことが、機械的に確認できた形になる。
