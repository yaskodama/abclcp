...
コーパスで境界を開いているのは \texttt{web\_calc.aipl} 1 本だけだった。
...
\texttt{s2\_missing\_reply.aipl} の \texttt{print(x + 1)} は
...
\begin{itemize}
\item \texttt{abclc/Hello.aipl} --- フィールドが
...
\item \texttt{s5\_overload\_ambiguity.aipl} --- 本体が \ty{float}、
...
\texttt{abclc/*.aipl} 63 本について、\texttt{git HEAD} 版と修正版の
型検査結果を比較した。
...
$ python3 scripts/type_runtime_diff.py --timeout 12 \
    abclc/*.aipl docs/samples/reply_inference/s*.aipl
...
\texttt{abclc/now\_future.aipl} も同じ形だったので \texttt{: int} を
注釈して静的・動的を揃えてある。

\subsection{型検査の退行}

変化した 1 本 \texttt{RotateThreeLines.aipl} は、
\begin{lstlisting}[language=AIPL]
...
曖昧警告が出た 2 本（\texttt{now\_future.aipl}, \texttt{web\_calc.aipl}）は
...
$ ./tc docs/samples/reply_inference/s1_ok.aipl
\end{lstlisting}

\subsection{s1\_ok.aipl --- reply からの推論}

\lstinputlisting[language=AIPL]{samples/reply_inference/s1_ok.aipl}
...
\subsection{s2\_missing\_reply.aipl --- reply の書き忘れ}

\lstinputlisting[language=AIPL]{samples/reply_inference/s2_missing_reply.aipl}
...
\subsection{s3\_conflict.aipl --- 分岐ごとに異なる型を reply}

\lstinputlisting[language=AIPL]{samples/reply_inference/s3_conflict.aipl}
...
\subsection{s4\_chain.aipl --- アクターを跨いだ伝播}

\lstinputlisting[language=AIPL]{samples/reply_inference/s4_chain.aipl}
...
\subsection{s5\_overload\_ambiguity.aipl --- principal type が無い例}

\lstinputlisting[language=AIPL]{samples/reply_inference/s5_overload_ambiguity.aipl}
...
\subsection{s6\_usable\_result.aipl --- 通るようになった正しいプログラム}

\lstinputlisting[language=AIPL]{samples/reply_inference/s6_usable_result.aipl}
...
\subsection{s7\_annotation\_ok.aipl --- 注釈が推論を助ける}

\lstinputlisting[language=AIPL]{samples/reply_inference/s7_annotation_ok.aipl}
...
\subsection{s8\_annotation\_conflict.aipl --- 宣言と食い違う reply}

\lstinputlisting[language=AIPL]{samples/reply_inference/s8_annotation_conflict.aipl}
...
\subsection{s9\_missing\_path.aipl --- 全パス被覆検査}

\lstinputlisting[language=AIPL]{samples/reply_inference/s9_missing_path.aipl}
...
\subsection{s10\_expose\_unannotated.aipl --- リモート境界での注釈必須}

\lstinputlisting[language=AIPL]{samples/reply_inference/s10_expose_unannotated.aipl}
...
\subsection{s11\_service.aipl --- 追加機能を一通り使う}

戻り値型注釈、全パス被覆検査、双方向型付け、\texttt{select} の $\rho$ 帰属、
そして注釈を省いたメソッドの推論を 1 本にまとめたもの。

\lstinputlisting[language=AIPL]{samples/reply_inference/s11_service.aipl}
...
\subsection{s12\_service\_exposed.aipl --- 同じものを外部公開する}
...
\subsection{s13\_runtime\_mismatch.aipl --- 型検査器と評価器の食い違い}

\lstinputlisting[language=AIPL]{samples/reply_inference/s13_runtime_mismatch.aipl}

\begin{lstlisting}[language={}]
$ ./tc docs/samples/reply_inference/s13_runtime_mismatch.aipl
[OK] type check passed
[method return types (inferred from reply)]
  T#f -> int
...
\subsection{s14\_select\_pattern.aipl --- select パターンの照合}

\lstinputlisting[language=AIPL]{samples/reply_inference/s14_select_pattern.aipl}
...
\subsection{s15\_double\_reply.aipl --- reply の線形性}

\lstinputlisting[language=AIPL]{samples/reply_inference/s15_double_reply.aipl}
...
\subsection{s16\_concat.aipl --- 文字列連結 \texttt{++} と数値 \texttt{+}}

\lstinputlisting[language=AIPL]{samples/reply_inference/s16_concat.aipl}
...
\item \texttt{abclc/*.aipl} ほか 38 ファイル / 255 箇所 --- \texttt{+} を
  \texttt{++} へ移行。
\item \texttt{src/infer.ml} --- 曖昧 overload を既定でエラーに、
  仮引数を署名へ結線（\S\ref{sec:wiring}）。
\item \texttt{abclc/Hello.aipl} --- \texttt{new Hello(5)} $\to$
  \texttt{new Hello(5.)}（結線で露見した int/float 不整合）。
\item \texttt{abclc/web\_calc.aipl}, \texttt{src/web\_calc.aipl},
  \texttt{src/web\_calc1.aipl} --- 公開アクターに戻り値型注釈を付けて移行。
\end{itemize}
