...
弾かれるべきプログラムは 31 本ある。
なお本稿が仕様として述べるのは、三実装すべてが備える\textbf{核}である ---
Python 版はこれに記録型・精緻化型・チャネル・所有などの\textbf{拡張}を持っており、
言語は二層になっている（\S\ref{sec:layers}）。
\textbf{デッドロック自由と効果の健全性}については\textbf{機械検証}まで進めた ---
Coq/Rocq で、\texttt{await} を含んだままの言語について
公理なしで証明してある（\S\ref{sec:coq}）。
\end{abstract}
...
デッドロック自由が型で保証される。}
この主張は紙の上だけではない ---
\S\ref{sec:coq} で、\texttt{await} を含んだままの中核について機械検証してある。
注釈は要らない --- 推論が埋める。
...
\subsection{三実装は同じ言語ではない --- 二層である}
\label{sec:layers}

ここは正確に書いておく必要がある。
「三実装は同期している」という言い方をしてきたが、
それは\textbf{正しいプログラムの出力について}の話であって、
\textbf{言語の機能そのものは揃っていない}。
行数の差（Python 版 15,101 行 対 OCaml 版 8,023 行）が、そのまま機能の差である。

そこで AIPL を\textbf{二層}として明示することにした。
...
層 & どこが実装するか & 内容 \\
\midrule
\textbf{核} & 三実装すべて & 本稿が述べている仕様 ---
  三種の送信・逐次性・\texttt{reply}・返信先の一級性・効果・期限・
  \texttt{result}$\langle\tau\rangle$・資源の順序・義務レベル・セッション型・メッシュ配備 \\
\addlinespace
\textbf{拡張} & Python 版のみ & 記録型と行多相、\texttt{where} による精緻化型、
  CSP チャネル、所有（\texttt{pub} / \texttt{move}）、タプル、\texttt{match}、
  saga、\texttt{scope \{ future ... \}}、\texttt{sender} を値として使うこと、
  および Python 版だけが持つ組込み \\
\bottomrule
\end{tabular}
\end{center}

\textbf{本稿が仕様として述べているのは核だけである。}
拡張は実験的な段階にあり、三実装で突き合わせる対象にしていない。

\paragraph{分類は機械的に決まる。}
どの層に属するかは、意見ではなく\textbf{どの処理系が受け付けるか}で決める。
...
\midrule
OCaml 版が通す & 核 \\
OCaml は落ちるが Python 版が通す & 拡張 \\
どちらも落ちる & 要修正（意図的な不正例を除く） \\
\bottomrule
\end{tabular}
\end{center}

手元の \texttt{.aipl} 586 本に当てると、
\textbf{核 148 / 拡張 306 / 意図的な不正例 47 / 要修正 86} である
（\texttt{tools/classify\_corpus.sh}、明細は \texttt{docs/corpus\_layers.tsv}）。

\paragraph{核では書けないもの。}
二層を分けてみて、核の輪郭がはっきりした。
次の三つは拡張でしか書けない。

\begin{itemize}
\item \textbf{返信先をフィールドに保持する}。
      引数として渡すことはできるが（\S\ref{sec:reply1st}）、
      フィールドは初期化子から型が決まるので
      \texttt{var w = 0;} は \texttt{int} になり、\texttt{w = replyto} が型不一致になる。
      「送り主を溜めて後で返す」有界バッファのような書き方が、核では直接には書けない。
\item \textbf{\texttt{self} / \texttt{sender} を値として使う}。
      送信の宛先としてしか書けない（\S\ref{sec:reply1st} の副産物）。
\item \textbf{他のアクターのフィールドへ代入する}。
      \texttt{w.x1 = ...} は書けない。カプセル化がそのまま型に出ている。
\end{itemize}
...
\subsection{機械検証 --- $\mathrm{AIPL}^{-2}$}
\label{sec:coq}

機械検証は Coq/Rocq 9.1.0 で行っている。二つの版がある。

\paragraph{第 1 版 $\mathrm{AIPL}^{-}$（1{,}121 行）。}
効果も期限も持たない中核について、
保存定理・進行定理・型安全性・\texttt{method not understood} の不在を示した。
デッドロック自由については\textbf{\texttt{await} を言語から取り除いた断片}
（非同期送信だけの断片）についてしか言えていなかった。
待てる構文を捨てて「待ちが返らない」を消していたのだから、当然である。

\paragraph{第 2 版 $\mathrm{AIPL}^{-2}$（1{,}524 行）。}
\texttt{await} を含んだままデッドロック自由を証明し、
併せて\textbf{効果の健全性}も証明した。
道具は本稿の\textbf{義務レベル}（\S\ref{sec:dl3}）と
\textbf{効果}（\S\ref{sec:eff}）そのものである。
まずデッドロック自由のために足したのは四つだけである。
...
足したもの & 内容 \\
\midrule
future の型にレベル & $\mathtt{TFut} : \tau \to \mathbb{N} \to \mathtt{ty}$ ---
  「この future を埋めるのはレベル $n$ のメソッドである」 \\
判断にレベル & $\Gamma \vdash_{C,L} e : \tau$ ---
  いま検査しているメソッドの義務レベル $L$ \\
待ちの規則 & $\Gamma \vdash_{C,L} e : \mathtt{future}\langle\tau\rangle@n$
  かつ $L < n$ ならば $\Gamma \vdash_{C,L} \mathtt{await}\ e : \tau$ \\
実行時の不変条件 & 未解決の future には必ず\textbf{埋める者}がいる ---
  メールボックスの中のメッセージか、走っているタスクである \\
\bottomrule
\end{tabular}
\end{center}

証明は「レベルが最大のタスクを取る」ことで進む。
全タスクが待ちだと仮定する。レベル最大のタスクも待っている。
待っている先の future はそれより真に上のレベルである（待ちの規則）。
その future には埋める者がいる（不変条件）。
メールボックスは空だから埋める者はタスクであり、そのレベルは上である。
レベル最大に反する。

その結果、第 1 版の進行定理から\textbf{枝が一つ消えた}。
...
\paragraph{効果も同じ形で載った。}
レベルを \texttt{future} の型に載せたのと\textbf{同じやり方}で、
効果も載せた ---- $\mathtt{future}\langle\tau\rangle@n\,!\,\varepsilon$。
規則の要は二つで、どちらも実装の挙動と同じである。
...
規則 & 効果 \\
\midrule
送信 & 呼び先の効果を\textbf{引き継がない}（待たないので）。
  ただし結果の型に記録する \\
待ち & 呼び先の効果を\textbf{引き継ぐ}。型に記録されていたものを課金する \\
\bottomrule
\end{tabular}
\end{center}

得られる定理は\textbf{メソッドが宣言した効果は、
\texttt{now} で呼んだ先も含めて、そのメソッドが実際に行うことを覆っている}
（\texttt{effect\_soundness}）。
系として、\texttt{ai} を宣言していないメソッドは
\texttt{ai} を持つメソッドを \texttt{await} できない ----
\S\ref{sec:intro} で述べた動機が、定理になった。

主定理は十三あり、いずれも \texttt{Print Assumptions} が
\texttt{Closed under the global context}（公理なし）である。

\subsection{証明できる最大の構文}

第 2 版を作るにあたって、\textbf{三つ（健全性・安全性・デッドロック自由）が
同時に成り立つ範囲はどこまでか}を機構ごとに調べた。

\paragraph{入れても保たれるもの。}
\texttt{while} と逐次実行を入れた。
止まらない繰り返しは「デッドロック」ではなく「停止しない」であって、
進行定理は一歩進めることしか言っていないので、三つとも保たれる。
\texttt{while} は \texttt{if} へ展開する形にしたので束縛を作らず、
変数捕獲の心配が無い。
動的なアクター生成と \texttt{future} の入れ子も入る ---
レベルは式の位置ではなくメソッドに付くので、入れ子でも同じ条件で足りる。
...
機構 & 壊れるもの \\
\midrule
\texttt{select} & 待つ相手が誰かは型に現れないので、レベルが効かない。
  進行の枝が増え、デッドロック自由が壊れる
  （実装では期限の義務づけと送り手の存在検査で補っている、\S\ref{sec:select3}） \\
\texttt{timeout} & 時間を意味論に入れる必要がある。進行は保てるが、
  主張が「詰まらない」から「詰まっても諦める」に変わる \\
\texttt{any}・多重定義・多相 & 標準形補題が壊れる。
  値の形が型から決まらないので進行が通らない \\
\texttt{become} & 受け手のクラスが実行時に変わると、
  配送時に型が合っている保証が崩れる \\
他アクターの状態への代入 & 「各アクターの状態は宣言型を持つ」が保てない \\
\bottomrule
\end{tabular}
\end{center}

\begin{finding}
この線引きは、\S\ref{sec:table} の採否の判断と\textbf{独立に}引いたのに、
ほぼ同じところに落ちた。
...
正直に書いておくべきことは残る。
機械検証されたのは\textbf{期限を持たない中核}である。
本稿の七つの規律のうち、機械検証まで届いたのは
\textbf{効果}と\textbf{デッドロック自由（義務レベル）}の二つで、
期限・\texttt{result}$\langle\tau\rangle$・返信先の線形性・
資源の順序・セッション型は紙の上の議論である。
...
\paragraph{空虚でないことの確認。}
形式化の定理はすべて「プログラムが型検査を通っている」という仮定のもとにある。
レベルの条件が満たせないなら定理は空虚なので、
\texttt{await} を実際に使い \texttt{while} も含むプログラムを一つ形式化し、
仮定をすべて満たすこと、初期構成が不変条件を満たすこと、
したがってデッドロック自由が適用できることを確かめた。
併せて\textbf{レベルを同じにすると本体の型検査が通らない}ことも確かめた ---
条件が実際に効いていることの対照である。
...
\paragraph{実例 --- 二つが正しく、正が間違っていた。}
次の 2 行は、Python 版と JavaScript 版が通し、\textbf{OCaml 版だけが弾いていた}。

\begin{aipl}
class Fork  { var held = 0; method take() : int !{mut} { held = 1; reply(held); } }
class Latch { var held = 0; method arm()  : int !{mut} { held = 2; reply(held); } }
\end{aipl}
\begin{term}
[Type error] cannot assign to overloaded name: held
\end{term}

\textbf{別のクラスが同じ名前のフィールドを持てなかった}のである。
原因は型検査器がフィールドを環境に「重ねて」入れていたことで、
二つ目のクラスの \texttt{held} が多重定義になり、代入が弾かれていた。
クラス本体はそのクラスのフィールドの下で検査するのだから、
重ねるのではなく置き換えるのが正しい。

手元の 586 本のうち\textbf{119 本}が、同じ名前のフィールドを持つ二つのクラスを含んでいた。
それでも長く気づかなかったのは、
ほとんどのファイルで\textbf{別のエラーが先に出て隠れていた}からである
（修正の前後で判定が変わったのは 2 本だけであった）。
仕様上の欠陥は、実際には\textbf{他の欠陥の陰に}いる。

\begin{finding}
「正」の実装を一つ決める方式は、\textbf{その実装の欠陥を検出できない}。
上の例を見つけられたのは、
同じプログラムを三つに投げて\textbf{二対一}になったからである。
証明も、単一実装のテストも、この形の誤りには届かない。
\end{finding}
...
\item \textbf{期限と、残る規律の機械検証}。
      効果とデッドロック自由（義務レベル）は $\mathrm{AIPL}^{-2}$ で
      機械検証まで届いた（\S\ref{sec:coq}）。
      期限・\texttt{result}$\langle\tau\rangle$・返信先の線形性・
      資源の順序・セッション型は紙の上の議論のままである。
      次は\textbf{期限}だが、時間を意味論に持ち込む必要があるので、
      効果ほど安くはない。
\item \textbf{実機のメッシュへの接続}。
      配備の輸送は現状ひとつのプロセス内で模している。
      Xinu ノード間の実際の転送に差し替える。
\item \textbf{拡張層の位置づけ}。
      \S\ref{sec:layers} で分けた拡張（記録型・精緻化型・チャネル・所有・saga など）は、
      Python 版だけが持つ実験的な段階にある。
      核へ昇格させるか、実験として畳むかを機構ごとに決める必要がある。
      判断の材料はここまでと同じであるべきだ ---
      反例が AIPL の言葉で出るか、証明の構造を壊さないか、実際に書かれているか。
\item \textbf{要修正の 86 本}。
      核でも拡張でも通らないプログラムが 86 本ある。
      多くは過去の実験の記録で、当時の処理系に対して書かれたものである。
\end{enumerate}
...
# 核 / 拡張 / 要修正 の分類（docs/AIPL_LAYERS.md に定義）
sh tools/classify_corpus.sh
\end{term}
