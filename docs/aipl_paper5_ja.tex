...
デッドロック自由については\textbf{機械検証}まで進めた ---
...
\paragraph{第 2 版 $\mathrm{AIPL}^{-2}$（1{,}314 行）。}
\texttt{await} を含んだままデッドロック自由を証明した。
道具は本稿の\textbf{義務レベル}（\S\ref{sec:dl3}）そのものである。
形式化に足したのは四つだけである。
...
主定理は九つあり、いずれも \texttt{Print Assumptions} が
...
正直に書いておくべきことは残る。
機械検証されたのは\textbf{効果と期限を持たない中核}である。
本稿の七つの規律のうち、機械検証まで届いたのは
\textbf{デッドロック自由（義務レベル）}だけで、
効果・期限・\texttt{result}$\langle\tau\rangle$・返信先の線形性・
...
\item \textbf{効果と期限の機械検証}。
      デッドロック自由（義務レベル）は $\mathrm{AIPL}^{-2}$ で機械検証まで届いたが
      （\S\ref{sec:coq}）、効果つき判断・期限・\texttt{result}$\langle\tau\rangle$・
      返信先の線形性・資源の順序・セッション型は紙の上の議論のままである。
      次に形式化するなら効果である ---
      判断に $\varepsilon$ を足すだけなので、動く部分が少ない。
