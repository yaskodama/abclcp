...
| セッション型 | `protocol_define`/`protocol_start`/`protocol_end` | やり取りの**順序**違反 |
...
1. **資源への全体順序**。`acquire`/`release` は対を見るが資源間に順序が無く、
   取得順序の食い違いによるデッドロックが残る。義務レベルを資源にも課すのが素直。
2. **アクターをまたぐセッションの静的検査**。いまは実行時任せ。
3. **弾かれるべきテストの実装差**: 29本×3実装＝87枠のうち **10枠**が食い違う
...
4. **機械検証の範囲**。`AIPLSoundness.v`（1121行・公理なし）が押さえているのは
